 ...
 * 
 * Filename:    lex.c
 * To Compile:  gcc driver.c <filename.c>
	...
	while (file_index <= file_length) 
	{
		...
		*/
		...
		{ 
          ...
          // If we aren't at the max length read in the character.
          temp_string[temp_index++] = input[file_index++];
				}
				else 
				{
		...
		// Logic for symbols
    // Simply check if the symbol exists
    // If it does then make it
    // Else throw an error
		else 
		{ 
      // :=
      ...
      // Only here if the value doesn't exist, throw and error and exit
			else 
				exitProgramAfterError(1);
